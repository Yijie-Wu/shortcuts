...
import {get_accotest_shortcuts} from "@/apis/shortcuts.js"
import {showMessage} from "@/utils/message.js";
import {getKey, getMouse} from "@/utils/common.js";


const search = ref("");
const accotest_shortcuts_store = useAllAccotestShortcutStore()

onMounted(() => {
  get_accotest_shortcuts().then(res => {
    accotest_shortcuts_store.setShortcuts(res.data)
  }).catch(err => {
    showMessage('error', '获取Accotest快捷键列表失败:'+ err.message);
  })
})


const filterTableData = computed(() =>
    accotest_shortcuts_store.shortcuts.filter(
        (data) =>
            !search.value ||
