...
import {get_vscode_shortcuts} from "@/apis/shortcuts.js"
import {showMessage} from "@/utils/message.js";
import {getKey, getMouse} from "@/utils/common.js";


const search = ref("");
const vscode_shortcuts_store = useAllVscodeShortcutStore()

onMounted(() => {
  get_vscode_shortcuts().then(res => {
    vscode_shortcuts_store.setShortcuts(res.data)
  }).catch(err => {
    showMessage('error', '获取Vscode快捷键列表失败:'+ err.message);
  })
})


const filterTableData = computed(() =>
    vscode_shortcuts_store.shortcuts.filter(
        (data) =>
            !search.value ||
